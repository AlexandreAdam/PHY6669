...
\[
        \eta_b = 5.5\times 10^{-10} \left( \frac{\Omega_b h^2}{0.02} \right)
\]
...
\]
On obtiens $X_e(a_{\text{dec}}) = 6.56\times 10^{-3}$ et le facteur d'échelle 
obtenu correspond 
au redshift $z_{\text{dec}} \simeq 1121$. Notons que la fraction d'ionization 
elle-même ne dépend que faiblement des paramètres cosmologiques. L'exponentielle 
joue un rôle beaucoup plus important (expression pour $S$). La réponse est comparable à
...
le facteur numérique de la réponse théorique pour la prochain sous-section.
...
\[
        \ln \mathcal{L}(\mathcal{\mathcal{D}} \mid \sigma_w;\bar{w}) \simeq 
...
\[
        ...
        \frac{1}{\sigma_w^{3}}\sum_{i=1}^{|\mathcal{D}|}(d_i - \bar{w})^{2} - 
        \frac{|\mathcal{D}|}{\sigma_w} = 0
\]
...
sur la déviation standard est 2 fois l'inverse coefficient de deuxième ordre dans 
...
\begin{align*}
        ...
        \sum_{i = 1}^{|\mathcal{D}|}(d_i -\bar{w})^{2}}
\end{align*}
\endgroup
