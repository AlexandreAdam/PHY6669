...
fraction d'électrons libres (dans l'Univers jeune, $X_e = 1$). On a 
...
peut négligé la radiation au moment de la recombinaison (ce n'est 
pas précis mais cela nous permet de simplifier énormément les expressions): 
\[
        H(a_{\text{rec}}) \simeq H_0  \Omega_m^{1/2} a_{\text{rec}}^{-3/2}
\]
...
\[
        \frac{\Gamma_T}{H} = 1 = 
                6.9108\times 10^{-2}\,\, \Omega_b^{1/2} h X_e a_{\text{rec}}^{-3/2}
\]
donc
\[
        a_{\text{rec}} = 0.6840\, (\Omega_b h^2X^2_e(a_{\text{rec}}))^{1/3}
\]
...
l'équation de Saha (qui cesse d'être valide précisément au moment qui nous intéresse).
